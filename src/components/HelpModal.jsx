...
import { FaCheck, FaTimes, FaHandPointer, FaMobileAlt, FaVolumeUp, FaLock } from "react-icons/fa";
    ...
    <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs flex items-center justify-center z-100 p-4 animate-fade-in">
      ...
      >
        ...
        <div className="p-6 py-4 flex flex-col gap-5 overflow-y-auto">
          {/* Card Legend with Lionel Messi (ARG17) */}
          <div>
            <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2.5">
              Estados de las Figuritas (Ej. Messi ARG17)
            </h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-white border border-slate-100 p-4.5 rounded-2xl shadow-xs justify-items-center">
              ...
              <div className="flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-bold text-slate-500 uppercase tracking-wide">Faltante</span>
                <div className="w-16 h-18 bg-white text-slate-400 border border-slate-200/85 rounded-xl flex flex-col items-center justify-center select-none shadow-xs">
                  ...
                  <span className="font-medium text-center text-[6px] px-1 mt-0.5 leading-tight opacity-85">Lionel Messi</span>
                </div>
              </div>

              {/* Obtenida */}
              <div className="flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-bold text-success uppercase tracking-wide">Obtenida</span>
              ...
              {/* Repetida */}
              <div className="flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-bold text-accent uppercase tracking-wide">Repetida</span>
              ...
              {/* Favorita */}
              <div className="flex flex-col items-center gap-1.5">
                <span className="text-[9px] font-bold text-amber-600 uppercase tracking-wide">Favorita</span>
          ...
          <div>
            ...
            <div className="flex flex-col gap-2.5">
              <div className="flex gap-3 bg-white p-3 rounded-2xl border border-slate-100 items-start">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-700 m-0 uppercase tracking-wide">1 Toque / Click Corto</h4>
                  <p className="text-[11px] text-slate-500 m-0 mt-0.5 leading-relaxed font-medium">
                    Marca la figurita como <strong>Obtenida</strong>. Si ya la tienes, le añade una <strong>Repetida</strong> (x1, x2, etc.).
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 bg-white p-3 rounded-2xl border border-slate-100 items-start">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-700 m-0 uppercase tracking-wide">Presionar 1 Segundo (Hold)</h4>
                  <p className="text-[11px] text-slate-500 m-0 mt-0.5 leading-relaxed font-medium">
                    <strong>Resta</strong> una repetida. Si llega a 0, la desmarca completamente volviéndola <strong>Faltante</strong>. Dispara vibración física.
                  </p>
                </div>
              </div>
              ...
              <div className="flex gap-3 bg-white p-3 rounded-2xl border border-slate-100 items-start">
                ...
                <div>
                  <h4 className="text-xs font-bold text-slate-700 m-0 uppercase tracking-wide">Doble Toque / Double Tap</h4>
                  <p className="text-[11px] text-slate-500 m-0 mt-0.5 leading-relaxed font-medium">
                    Marca o desmarca la figurita como <strong>Favorita</strong> (destaca el cromo con un borde doble dorado). Dispara vibración física.
                  </p>
                </div>
              </div>
            </div>
          ...
          <div className="bg-primary/5 border border-primary/10 p-3.5 rounded-2xl">
            <h4 className="text-xs font-bold text-primary m-0 uppercase tracking-wide">👥 Álbum Único en Tiempo Real</h4>
            <p className="text-[11px] text-slate-600 m-0 mt-1 leading-relaxed font-medium">
              Puedes invitar a otra persona compartiendo el código de tu sala. Cualquier cambio que realicen se reflejará en ambos teléfonos con menos de 100ms de latencia.
            </p>
          </div>
          ...
          {isRoomNotSet && (
            <div className="bg-amber-500/5 border border-amber-500/20 p-4.5 rounded-2xl flex flex-col gap-2.5">
              <h4 className="text-xs font-bold text-slate-700 m-0 uppercase tracking-wide flex items-center gap-1.5">
                <FaLock className="text-amber-500 text-[10px]" /> Configura tu Sala Privada
              </h4>
              <p className="text-[11px] text-slate-500 m-0 leading-relaxed font-medium">
                Para evitar compartir el mismo álbum con otros usuarios públicos, ingresa un nombre o código único para tu sala (ej. <em>nuestro_album_2026</em>).
              </p>
                ...
                <div className="min-h-4.5 mt-1">
                  ...
                  {!isCheckingRoom && roomExists === true && (
                    <span className="text-[10px] text-blue-600 font-semibold flex items-center gap-1">
                      ℹ️ Esta sala ya existe. Te unirás al álbum compartido existente.
                    </span>
                  )}
                  {!isCheckingRoom && roomExists === false && (
                    <span className="text-[10px] text-emerald-600 font-semibold flex items-center gap-1">
                      ✨ Nombre disponible. Se creará un nuevo álbum privado.
                    </span>
                  )}
                </div>
              </div>
              ...
              {/* Security Warning */}
              <p className="text-[10px] text-red-500 font-semibold leading-relaxed m-0 mt-0.5">
                ⚠️ No compartas este código con extraños. Cualquiera que tenga el código de tu sala podrá ver y modificar tu álbum.
              </p>
            </div>
          )}
        </div>
        ...
        <div className="p-5 border-t border-slate-200 flex justify-center bg-white rounded-b-3xl">
          <button
            ...
            disabled={isRoomNotSet && (!roomInput.trim() || roomInput.trim() === "sala_predeterminada")}
            className={`w-full max-w-xs font-montserrat text-xs uppercase tracking-wider font-extrabold py-3 px-6 rounded-xl transition-all duration-300 ${
              isRoomNotSet && (!roomInput.trim() || roomInput.trim() === "sala_predeterminada")
