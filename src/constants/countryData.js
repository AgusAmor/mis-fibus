...
// Selection metadata including official display names and confederations
...
// Builds a dynamic, randomized (but deterministic) flag-colored background pattern (SVG)
export const getCountryPattern = (countryKey) => {
  ...
  // Deterministic hash helper to generate consistent random variation values per country
  const hash = (seed) => {
    let val = 0;
  ...
  const r1 = (hash("a") % 30) + 10;  // 10-40 range
  const r2 = (hash("b") % 40) + 30;  // 30-70 range
  const r3 = (hash("c") % 30) + 60;  // 60-90 range
  
  const cp1 = (hash("d") % 40) + 20; // 20-60 range
  const cp2 = (hash("e") % 40) + 40; // 40-80 range

  const curveType = hash("f") % 3; // 0, 1, or 2 for style path layout variation

  let paths = "";
  // Fill the base block in c1 (first flag color)
...
// Map of 3-letter country keys to their corresponding standard flag emoji
